...
const SAMPLE_ALBUM_DATA = {
    ...
    album: {
        description: "A collection of memories from Nani's childhood, wedding, and life lessons.",
...
export default function SampleAlbum() {
    ...
    const handleNextTrack = () => {
        if (playingTrackIndex !== null && playingTrackIndex < SAMPLE_ALBUM_DATA.tracks.length - 1) {
    ...
    const currentTrack = playingTrackIndex !== null ? SAMPLE_ALBUM_DATA.tracks[playingTrackIndex] : null;
        ...
        <div className="min-h-screen bg-[#FDF4DC] pb-[100px] relative">
                ...
                <div className="flex items-center gap-2">
                    ...
                    {isLanguageDropdownOpen && (
                        <div className="absolute top-full right-4 mt-2 bg-white rounded-xl shadow-xl border border-black/5 overflow-hidden min-w-[120px]">
                            ...
                            <button
                                onClick={() => { setSelectedLanguage("hi"); setIsLanguageDropdownOpen(false); }}
                                className="w-full text-left px-4 py-2 hover:bg-[#A35139]/10 text-sm font-['Outfit']"
                            >
                                हिंदी
                            </button>
            ...
            <div className="pt-8 px-6 pb-6 text-center">
                <div className="w-40 h-40 mx-auto rounded-2xl overflow-hidden shadow-2xl mb-6 relative group">
                    <img src={SAMPLE_ALBUM_DATA.album.coverImage} alt="Album Cover" className="w-full h-full object-cover" />
                </div>
                    ...
                    <div key={batchIndex} className="space-y-4">
                        ...
                        <div className="space-y-3">
                            {batch.tracks.map((track) => {
                                ...
                                const absoluteIndex = SAMPLE_ALBUM_DATA.tracks.findIndex(t => t.questionIndex === track.questionIndex);
                           ...
                           `}
                              ...
                              `}>
                                                {isTrackPlaying && isPlaying ? <Pause className="w-5 h-5" /> : <Play className="w-5 h-5 ml-0.5" />}
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                <p className={`font-['Outfit'] font-medium text-base mb-1 ${isTrackPlaying ? "text-[#A35139]" : "text-[#1B2632]"}`}>
                                                    {selectedLanguage === 'hi' ? "प्रश्न " : "Question "}{absoluteIndex + 1}
                                                </p>
                                            ...
                                            <div className="text-xs font-medium text-[#1B2632]/40 mt-1">
                                                {durations.get(absoluteIndex) ?
                                                    `${Math.floor(durations.get(absoluteIndex)! / 60)}:${Math.floor(durations.get(absoluteIndex)! % 60).toString().padStart(2, '0')}`
                                                    : "--:--"
                                                }
                                            </div>
                                        </div>
                ...
                <div className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-black/5 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] px-4 py-3 pb-8 sm:pb-3">
                    <div className="max-w-3xl mx-auto flex items-center justify-between gap-4">
                        <div className="flex items-center gap-3 min-w-0 flex-1">
                            <div className="w-12 h-12 rounded-lg bg-gray-200 overflow-hidden flex-shrink-0">
                                <img src={SAMPLE_ALBUM_DATA.album.coverImage} className="w-full h-full object-cover" />
                            </div>
                            <div className="min-w-0">
                                <p className="font-['Outfit'] font-bold text-[#1B2632] truncate text-sm">
                                    {selectedLanguage === 'hi' ? "प्रश्न " : "Question "}{playingTrackIndex! + 1}
                                </p>
                        ...
                        <div className="flex items-center gap-2 sm:gap-4">
                            <button onClick={handlePreviousTrack} className="p-2 text-[#1B2632]/40 hover:text-[#1B2632]">
                            ...
                            <button onClick={handleNextTrack} className="p-2 text-[#1B2632]/40 hover:text-[#1B2632]">
                    ...
                    <div className="absolute top-0 left-0 right-0 h-1 bg-black/5">
                        <div
                            ...
                            style={{ width: `${(currentTime / (durations.get(playingTrackIndex!) || 1)) * 100}%` }}
                        />
                    </div>
                </div>
            )}
