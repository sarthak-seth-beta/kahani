...
import { Play, Share2, Loader2, Heart, Copy, Mail, MessageCircle, ChevronLeft, ChevronRight, ArrowRight } from "lucide-react";
...
const AlbumCard = ({ album, onClick }: { album: Album; onClick: () => void }) => {
    ...
    const handleShare = (method: string) => {
        ...
        switch (method) {
            ...
            case 'native':
                if (navigator.share) {
                    navigator.share({ title: album.title, text: text, url: url }).catch(() => { });
                }
                break;
        }
    };
        ...
        >
            ...
            <div className="relative h-[65%] w-full flex-shrink-0 overflow-hidden">
                ...
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-48 bg-white/95 backdrop-blur-sm border-[#1B2632]/10 shadow-xl rounded-xl">
                        <DropdownMenuItem onClick={(e) => { e.stopPropagation(); handleShare('copy'); }} className="cursor-pointer">
                            <Copy className="mr-2 h-4 w-4" /> Copy link
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={(e) => { e.stopPropagation(); handleShare('whatsapp'); }} className="cursor-pointer">
                            <MessageCircle className="mr-2 h-4 w-4" /> Whatsapp
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={(e) => { e.stopPropagation(); handleShare('email'); }} className="cursor-pointer">
                            <Mail className="mr-2 h-4 w-4" /> Email
                        </DropdownMenuItem>
                        {typeof navigator.share === 'function' && (
                            <DropdownMenuItem onClick={(e) => { e.stopPropagation(); handleShare('native'); }} className="cursor-pointer">
                    ...
                    >
                        <Heart
                            className={cn(
                                ...
                                isLiked ? "fill-[#E5484D] text-[#E5484D]" : "text-[#1B2632]/30 hover:text-[#E5484D]"
                            )}
                        />
                    </button>
                </div>
                    ...
                    >
                        <div className="flex gap-6 w-max px-[7.5vw] sm:px-0 sm:mx-auto">
                            ...
                            >
                                ...
                                <h3 className="text-2xl font-bold text-[#1B2632] font-['Outfit'] mb-2">View All</h3>
                                <p className="text-[#1B2632]/60 text-sm font-medium">Browse our full collection</p>
                            </div>
