...
function Card({ item, setLocation }: { item: any; setLocation: any }) {
    ...
    const getGradient = (id: string, type: string) => {
        if (type === "coming-soon") return "bg-gradient-to-br from-[#EEE9DF] to-[#C9C1B1]"; // Palladian -> Oatmeal

        // Female relations (Warm): Truffle Trouble -> Burning Flame
        if (["mom", "nani", "dadi", "mom_2", "wife", "grandchild"].some(k => id.includes(k))) {
        ...
        >
            ...
            */}
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center p-4 text-center">
                <span className={`font-['Outfit'] font-bold text-xl md:text-2xl tracking-wide ${item.type === "coming-soon" ? "text-gray-400" : "text-white"}`}>
                    {item.label}
                </span>
